...
public:
	...
	void AddToolBar();
	SString TranslatePath(const char *path); //SciTEGTK::
...
SciTEGTK::SciTEGTK(Extension *ext) : SciTEBase(ext) {
	...
	exitStatus = 0;
	pollID = 0;
	sprintf(resultsFile, "/tmp/sdlBasic%x.results",
...
void SciTEGTK::GetDefaultDirectory(char *directory, size_t size) {
	directory[0] = '\0';
	char *where = getenv("SdlBasic_HOME");
#ifdef SYSCONF_PATH
	if (!where) {
		where = (char *)SYSCONF_PATH;
	}
#else
	if (!where) {
		where = getenv("HOME");
...
void SciTEGTK::Execute() {
	...
	if (jobQueue[icmd].jobType == jobShell) {
		printf("ciao!!!\n");
		if (fork() == 0)
			pidShell =execlp("/bin/sh", "sh", "-c", jobQueue[icmd].command.c_str(), NULL);
		else
			ExecuteNext();
...
void SciTEGTK::GoLineDialog() {
	...
	GtkWidget *table = gtk_table_new(2, 1, TRUE);//false
...
int SciTEGTK::WindowMessageBox(Window &w, const SString &msg, int style) {
	if (!messageBoxDialog) {
		...
		if (style & MB_ABOUTBOX) {
			...
			SetAboutMessage(explanation, "sdlBasic IDE  ");
		} else {
...
void SciTEGTK::AboutDialog() {
	WindowMessageBox(wSciTE, "sdlBasic editor based on SciTE\nby Neil Hodgson neilh@scintilla.org .",
	                 MB_OK | MB_ABOUTBOX);
}
...
#ifndef NO_FILER
		int fdPipe = props.GetInt("sdlbasic.ipc_fdpipe");
		if (fdPipe != -1) {
			close(fdPipe);
			unlink(props.Get("sdlbasic.ipc_name").c_str());
		}
#else
...
void SciTEGTK::AddToolBar() {
	if (props.GetInt("toolbar.usestockicons") == 1) {
		...
		gtk_toolbar_append_space(GTK_TOOLBAR(PWidget(wToolBar)));
		btnGo = AddToolButton("Go", IDM_GO, gtk_image_new_from_stock("gtk-convert", GTK_ICON_SIZE_LARGE_TOOLBAR));
		...
		gtk_toolbar_append_space(GTK_TOOLBAR(PWidget(wToolBar)));
		//AddToolButton("Clear All Bookmarks", IDM_BOOKMARK_CLEARALL, pixmap_new(PWidget(wSciTE), build_xpm));
		AddToolButton("Toggle Bookmark", IDM_BOOKMARK_TOGGLE, pixmap_new(PWidget(wSciTE), (gchar **)togglebookmark_xpm));
		AddToolButton("Previous Bookmark", IDM_BOOKMARK_PREV, pixmap_new(PWidget(wSciTE), (gchar **)previousbookmark_xpm));
		AddToolButton("Next Bookmark", IDM_BOOKMARK_NEXT, pixmap_new(PWidget(wSciTE), (gchar **)nextbookmark_xpm));
		// go - stop buttons
		gtk_toolbar_append_space(GTK_TOOLBAR(PWidget(wToolBar)));
		btnGo = AddToolButton("Go", IDM_GO, pixmap_new(PWidget(wSciTE), (gchar **)go_xpm));
...
void SciTEGTK::CreateMenu() {
	...
	GtkItemFactoryCallback menuSig = GtkItemFactoryCallback(MenuSignal);
		SciTEItemFactoryEntry menuItems[] = {
	                                      ...
	                                      {(gchar *)"/File/_New", (gchar *)"<control>N", menuSig, IDM_NEW, 0},
	                                      {(gchar *)"/File/_Open...", (gchar *)"<control>O", menuSig, IDM_OPEN, 0},
	                                      {(gchar *)"/File/Open Selected _Filename", (gchar *)"<control><shift>O", menuSig, IDM_OPENSELECTED, 0},
	                                      {(gchar *)"/File/_Revert", (gchar *)"<control>R", menuSig, IDM_REVERT, 0},
	                                      {(gchar *)"/File/_Close", (gchar *)"<control>W", menuSig, IDM_CLOSE, 0},
	                                      {(gchar *)"/File/_Save", (gchar *)"<control>S", menuSig, IDM_SAVE, 0},
	                                      {(gchar *)"/File/Save _As...", (gchar *)"<control><shift>S", menuSig, IDM_SAVEAS, 0},
	                                      {(gchar *)"/File/Save a Co_py...", (gchar *)"<control><shift>P", menuSig, IDM_SAVEACOPY, 0},
	                                      {(gchar *)"/File/Encodin_g", NULL, NULL, 0, (gchar *)"<Branch>"},
	                                      {(gchar *)"/File/Encoding/_8 Bit", NULL, menuSig, IDM_ENCODING_DEFAULT, (gchar *)"<RadioItem>"},
	                                      {(gchar *)"/File/Encoding/UCS-2 _Big Endian", NULL, menuSig, IDM_ENCODING_UCS2BE, (gchar *)"/File/Encoding/8 Bit"},
	                                      {(gchar *)"/File/Encoding/UCS-2 _Little Endian", NULL, menuSig, IDM_ENCODING_UCS2LE, (gchar *)"/File/Encoding/8 Bit"},
	                                      {(gchar *)"/File/Encoding/_UTF-8", NULL, menuSig, IDM_ENCODING_UTF8, (gchar *)"/File/Encoding/8 Bit"},
	                                      {(gchar *)"/File/Encoding/UTF-8 _Cookie", NULL, menuSig, IDM_ENCODING_UCOOKIE, (gchar *)"/File/Encoding/8 Bit"},
	                                      {(gchar *)"/File/_Export", (gchar *)"", 0, 0, (gchar *)"<Branch>"},
	                                      {(gchar *)"/File/Export/As _HTML...", NULL, menuSig, IDM_SAVEASHTML, 0},
	                                      {(gchar *)"/File/Export/As _RTF...", NULL, menuSig, IDM_SAVEASRTF, 0},
	                                      {(gchar *)"/File/Export/As _PDF...", NULL, menuSig, IDM_SAVEASPDF, 0},
	                                      {(gchar *)"/File/Export/As _LaTeX...", NULL, menuSig, IDM_SAVEASTEX, 0},
	                                      {(gchar *)"/File/Export/As _XML...", NULL, menuSig, IDM_SAVEASXML, 0},
	                                      {(gchar *)"/File/_Print", (gchar *)"<control>P", menuSig, IDM_PRINT, 0},
	                                      {(gchar *)"/File/sep1", NULL, NULL, 0, (gchar *)"<Separator>"},
	                                      {(gchar *)"/File/Load Session...", (gchar *)"", menuSig, IDM_LOADSESSION, 0},
	                                      {(gchar *)"/File/Save Session...", (gchar *)"", menuSig, IDM_SAVESESSION, 0},
	                                      {(gchar *)"/File/sep2", NULL, menuSig, IDM_MRU_SEP, (gchar *)"<Separator>"},
	                                      {(gchar *)"/File/File0", (gchar *)"", menuSig, fileStackCmdID + 0, 0},
	                                      {(gchar *)"/File/File1", (gchar *)"", menuSig, fileStackCmdID + 1, 0},
	                                      {(gchar *)"/File/File2", (gchar *)"", menuSig, fileStackCmdID + 2, 0},
	                                      {(gchar *)"/File/File3", (gchar *)"", menuSig, fileStackCmdID + 3, 0},
	                                      {(gchar *)"/File/File4", (gchar *)"", menuSig, fileStackCmdID + 4, 0},
	                                      {(gchar *)"/File/File5", (gchar *)"", menuSig, fileStackCmdID + 5, 0},
	                                      {(gchar *)"/File/File6", (gchar *)"", menuSig, fileStackCmdID + 6, 0},
	                                      {(gchar *)"/File/File7", (gchar *)"", menuSig, fileStackCmdID + 7, 0},
	                                      {(gchar *)"/File/File8", (gchar *)"", menuSig, fileStackCmdID + 8, 0},
	                                      {(gchar *)"/File/File9", (gchar *)"", menuSig, fileStackCmdID + 9, 0},
	                                      {(gchar *)"/File/sep3", NULL, NULL, 0, (gchar *)"<Separator>"},
	                                      {(gchar *)"/File/E_xit", (gchar *)"", menuSig, IDM_QUIT, 0},

	                                      {(gchar *)"/_Edit", NULL, NULL, 0, (gchar *)"<Branch>"},
	                                      {(gchar *)"/Edit/_Undo", (gchar *)"<control>Z", menuSig, IDM_UNDO, 0},

	                                      {(gchar *)"/Edit/_Redo", (gchar *)"<control>Y", menuSig, IDM_REDO, 0},
	                                      {(gchar *)"/Edit/sep1", NULL, NULL, 0, (gchar *)"<Separator>"},
	                                      {(gchar *)"/Edit/Cu_t", (gchar *)"<control>X", menuSig, IDM_CUT, 0},
	                                      {(gchar *)"/Edit/_Copy", (gchar *)"<control>C", menuSig, IDM_COPY, 0},
	                                      {(gchar *)"/Edit/_Paste", (gchar *)"<control>V", menuSig, IDM_PASTE, 0},
	                                      {(gchar *)"/Edit/_Delete", (gchar *)"Del", menuSig, IDM_CLEAR, 0},
	                                      {(gchar *)"/Edit/Select _All", (gchar *)"<control>A", menuSig, IDM_SELECTALL, 0},
	                                      {(gchar *)"/Edit/sep2", NULL, NULL, 0, (gchar *)"<Separator>"},
	                                      {(gchar *)"/Edit/Match _Brace", (gchar *)"<control>E", menuSig, IDM_MATCHBRACE, 0},
	                                      {(gchar *)"/Edit/Select t_o Brace", (gchar *)"<control><shift>E", menuSig, IDM_SELECTTOBRACE, 0},
	                                      {(gchar *)"/Edit/S_how Calltip", (gchar *)"<control>T", menuSig, IDM_SHOWCALLTIP, 0},
	                                      {(gchar *)"/Edit/sep2", NULL, NULL, 0, (gchar *)"<Separator>"},
					      {(gchar *)"/Edit/Complete S_ymbol", (gchar *)"<control>I", menuSig, IDM_COMPLETE, 0},
	                                      {(gchar *)"/Edit/Complete _Word", (gchar *)"<control>Return", menuSig, IDM_COMPLETEWORD, 0},
	                                      {(gchar *)"/Edit/_Expand Abbreviation", (gchar *)"<control>B", menuSig, IDM_ABBREV, 0},
	                                      //{"/Edit/_Insert Abbreviation", "<control>D", menuSig, IDM_INS_ABBREV, 0},
	                                      {(gchar *)"/Edit/sep2", NULL, NULL, 0, (gchar *)"<Separator>"},
					      {(gchar *)"/Edit/Block Co_mment or Uncomment", (gchar *)"<control>Q", menuSig, IDM_BLOCK_COMMENT, 0},
	                                      ...
	                                      {(gchar *)"/Edit/Make _Selection Uppercase", (gchar *)"<control>U", menuSig, IDM_UPRCASE, 0},
	                                      {(gchar *)"/Edit/Make Selection _Lowercase", (gchar *)"<control>L", menuSig, IDM_LWRCASE, 0},
	                                      ...
	                                      {(gchar *)"/_Search", NULL, NULL, 0, (gchar *)"<Branch>"},
	                                      {(gchar *)"/Search/_Find...", (gchar *)"<control>F", menuSig, IDM_FIND, 0},
	                                      {(gchar *)"/Search/Find _Next", (gchar *)"F3", menuSig, IDM_FINDNEXT, 0},
	                                      {(gchar *)"/Search/Find Previou_s", (gchar *)"<shift>F3", menuSig, IDM_FINDNEXTBACK, 0},
	                                      {(gchar *)"/Search/F_ind in Files...", (gchar *)"<control><shift>F", menuSig, IDM_FINDINFILES, 0},
	                                      {(gchar *)"/Search/R_eplace...", (gchar *)"<control>H", menuSig, IDM_REPLACE, 0},
#ifdef INCREMENTAL_SEARCH
	                                      {(gchar *)"/Search/Incrementa&l Search", (gchar *)"<control><alt>I", menuSig, IDM_INCSEARCH, 0},
#endif
	                                      {(gchar *)"/Search/sep3", NULL, NULL, 0, (gchar *)"<Separator>"},
	                                      {(gchar *)"/Search/_Go To...", (gchar *)"<control>G", menuSig, IDM_GOTO, 0},
	                                      {(gchar *)"/Search/Next Book_mark", (gchar *)"F2", menuSig, IDM_BOOKMARK_NEXT, 0},
	                                      {(gchar *)"/Search/Pre_vious Bookmark", (gchar *)"<shift>F2", menuSig, IDM_BOOKMARK_PREV, 0},
	                                      {(gchar *)"/Search/Toggle Bookmar_k", (gchar *)"<control>M", menuSig, IDM_BOOKMARK_TOGGLE, 0},
	                                      {(gchar *)"/Search/_Clear All Bookmarks", (gchar *)"", menuSig, IDM_BOOKMARK_CLEARALL, 0},

	                                      {(gchar *)"/_View", NULL, NULL, 0, (gchar *)"<Branch>"},
	                                      {(gchar *)"/View/Toggle _current fold", (gchar *)"", menuSig, IDM_EXPAND, 0},
	                                      {(gchar *)"/View/Toggle _all folds", (gchar *)"", menuSig, IDM_TOGGLE_FOLDALL, 0},
	                                      {(gchar *)"/View/sep1", NULL, NULL, 0, (gchar *)"<Separator>"},
	                                      {(gchar *)"/View/Full Scree_n", (gchar *)"F11", menuSig, IDM_FULLSCREEN, (gchar *)"<CheckItem>"},
	                                      {(gchar *)"/View/_Tool Bar", (gchar *)"", menuSig, IDM_VIEWTOOLBAR, (gchar *)"<CheckItem>"},
#if GTK_MAJOR_VERSION >= 2
	                                      {(gchar *)"/View/Tab _Bar", (gchar *)"", menuSig, IDM_VIEWTABBAR, (gchar *)"<CheckItem>"},
#endif
	                                      {(gchar *)"/View/_Status Bar", (gchar *)"", menuSig, IDM_VIEWSTATUSBAR, (gchar *)"<CheckItem>"},
	                                      {(gchar *)"/View/sep2", NULL, NULL, 0, (gchar *)"<Separator>"},
	                                      {(gchar *)"/View/_Whitespace", (gchar *)"<control><shift>A", menuSig, IDM_VIEWSPACE, (gchar *)"<CheckItem>"},
	                                      {(gchar *)"/View/_End of Line", (gchar *)"<control><shift>D", menuSig, IDM_VIEWEOL, (gchar *)"<CheckItem>"},
	                                      {(gchar *)"/View/_Indentation Guides", NULL, menuSig, IDM_VIEWGUIDES, (gchar *)"<CheckItem>"},
	                                      {(gchar *)"/View/_Line Numbers", (gchar *)"", menuSig, IDM_LINENUMBERMARGIN, (gchar *)"<CheckItem>"},
	                                      {(gchar *)"/View/_Margin", NULL, menuSig, IDM_SELMARGIN, (gchar *)"<CheckItem>"},
	                                      {(gchar *)"/View/_Fold Margin", NULL, menuSig, IDM_FOLDMARGIN, (gchar *)"<CheckItem>"},
	                                      //{"/View/_Output", "F8", menuSig, IDM_TOGGLEOUTPUT, "<CheckItem>"},
	                                      //{"/View/_Parameters", NULL, menuSig, IDM_TOGGLEPARAMETERS, "<CheckItem>"},

	                                      {(gchar *)"/_Tools", NULL, NULL, 0, (gchar *)"<Branch>"},
	                                      {(gchar *)"/Tools/_Go", (gchar *)"F5", menuSig, IDM_GO, 0},
					      {(gchar *)"/Tools/_Stop Executing", (gchar *)"<control>.", menuSig, IDM_STOPEXECUTE, NULL},

	                                      {(gchar *)"/Tools/sep1", NULL, NULL, 0, (gchar *)"<Separator>"},
	                                      ...
	                                      {(gchar *)"/Tools/_Switch Pane", (gchar *)"<control>F6", menuSig, IDM_SWITCHPANE, 0},

					      {(gchar *)"/_Plugins", NULL, NULL, 0, (gchar *)"<Branch>"},

	                                      {(gchar *)"/Plugins/Tool0", NULL, menuSig, IDM_TOOLS + 0, 0},
	                                      {(gchar *)"/Plugins/Tool1", NULL, menuSig, IDM_TOOLS + 1, 0},
	                                      {(gchar *)"/Plugins/Tool2", NULL, menuSig, IDM_TOOLS + 2, 0},
	                                      {(gchar *)"/Plugins/Tool3", NULL, menuSig, IDM_TOOLS + 3, 0},
	                                      {(gchar *)"/Plugins/Tool4", NULL, menuSig, IDM_TOOLS + 4, 0},
	                                      {(gchar *)"/Plugins/Tool5", NULL, menuSig, IDM_TOOLS + 5, 0},
	                                      {(gchar *)"/Plugins/Tool6", NULL, menuSig, IDM_TOOLS + 6, 0},
	                                      {(gchar *)"/Plugins/Tool7", NULL, menuSig, IDM_TOOLS + 7, 0},
	                                      {(gchar *)"/Plugins/Tool8", NULL, menuSig, IDM_TOOLS + 8, 0},
	                                      {(gchar *)"/Plugins/Tool9", NULL, menuSig, IDM_TOOLS + 9, 0},

	                                      {(gchar *)"/Plugins/Tool10", NULL, menuSig, IDM_TOOLS + 10, 0},
	                                      {(gchar *)"/Plugins/Tool11", NULL, menuSig, IDM_TOOLS + 11, 0},
	                                      {(gchar *)"/Plugins/Tool12", NULL, menuSig, IDM_TOOLS + 12, 0},
	                                      {(gchar *)"/Plugins/Tool13", NULL, menuSig, IDM_TOOLS + 13, 0},
	                                      {(gchar *)"/Plugins/Tool14", NULL, menuSig, IDM_TOOLS + 14, 0},
	                                      {(gchar *)"/Plugins/Tool15", NULL, menuSig, IDM_TOOLS + 15, 0},
	                                      {(gchar *)"/Plugins/Tool16", NULL, menuSig, IDM_TOOLS + 16, 0},
	                                      {(gchar *)"/Plugins/Tool17", NULL, menuSig, IDM_TOOLS + 17, 0},
	                                      {(gchar *)"/Plugins/Tool18", NULL, menuSig, IDM_TOOLS + 18, 0},
	                                      {(gchar *)"/Plugins/Tool19", NULL, menuSig, IDM_TOOLS + 19, 0},

	                                      {(gchar *)"/Plugins/Tool20", NULL, menuSig, IDM_TOOLS + 20, 0},
	                                      {(gchar *)"/Plugins/Tool21", NULL, menuSig, IDM_TOOLS + 21, 0},
	                                      {(gchar *)"/Plugins/Tool22", NULL, menuSig, IDM_TOOLS + 22, 0},
	                                      {(gchar *)"/Plugins/Tool23", NULL, menuSig, IDM_TOOLS + 23, 0},
	                                      {(gchar *)"/Plugins/Tool24", NULL, menuSig, IDM_TOOLS + 24, 0},
	                                      {(gchar *)"/Plugins/Tool25", NULL, menuSig, IDM_TOOLS + 25, 0},
	                                      {(gchar *)"/Plugins/Tool26", NULL, menuSig, IDM_TOOLS + 26, 0},
	                                      {(gchar *)"/Plugins/Tool27", NULL, menuSig, IDM_TOOLS + 27, 0},
	                                      {(gchar *)"/Plugins/Tool28", NULL, menuSig, IDM_TOOLS + 28, 0},
	                                      {(gchar *)"/Plugins/Tool29", NULL, menuSig, IDM_TOOLS + 29, 0},

	                                      {(gchar *)"/Plugins/Tool30", NULL, menuSig, IDM_TOOLS + 30, 0},
	                                      {(gchar *)"/Plugins/Tool31", NULL, menuSig, IDM_TOOLS + 31, 0},
	                                      {(gchar *)"/Plugins/Tool32", NULL, menuSig, IDM_TOOLS + 32, 0},
	                                      {(gchar *)"/Plugins/Tool33", NULL, menuSig, IDM_TOOLS + 33, 0},
	                                      {(gchar *)"/Plugins/Tool34", NULL, menuSig, IDM_TOOLS + 34, 0},
	                                      {(gchar *)"/Plugins/Tool35", NULL, menuSig, IDM_TOOLS + 35, 0},
	                                      {(gchar *)"/Plugins/Tool36", NULL, menuSig, IDM_TOOLS + 36, 0},
	                                      {(gchar *)"/Plugins/Tool37", NULL, menuSig, IDM_TOOLS + 37, 0},
	                                      {(gchar *)"/Plugins/Tool38", NULL, menuSig, IDM_TOOLS + 38, 0},
	                                      {(gchar *)"/Plugins/Tool39", NULL, menuSig, IDM_TOOLS + 39, 0},

	                                      {(gchar *)"/Plugins/Tool40", NULL, menuSig, IDM_TOOLS + 40, 0},
	                                      {(gchar *)"/Plugins/Tool41", NULL, menuSig, IDM_TOOLS + 41, 0},
	                                      {(gchar *)"/Plugins/Tool42", NULL, menuSig, IDM_TOOLS + 42, 0},
	                                      {(gchar *)"/Plugins/Tool43", NULL, menuSig, IDM_TOOLS + 43, 0},
	                                      {(gchar *)"/Plugins/Tool44", NULL, menuSig, IDM_TOOLS + 44, 0},
	                                      {(gchar *)"/Plugins/Tool45", NULL, menuSig, IDM_TOOLS + 45, 0},
	                                      {(gchar *)"/Plugins/Tool46", NULL, menuSig, IDM_TOOLS + 46, 0},
	                                      {(gchar *)"/Plugins/Tool47", NULL, menuSig, IDM_TOOLS + 47, 0},
	                                      {(gchar *)"/Plugins/Tool48", NULL, menuSig, IDM_TOOLS + 48, 0},
	                                      {(gchar *)"/Plugins/Tool49", NULL, menuSig, IDM_TOOLS + 49, 0},

	                                  };

	SciTEItemFactoryEntry menuItemsOptions[] = {
	            {(gchar *)"/_Options", NULL, NULL, 0, (gchar *)"<Branch>"},
	            {(gchar *)"/Options/Vertical _Split", (gchar *)"", menuSig, IDM_SPLITVERTICAL, (gchar *)"<CheckItem>"},
	            {(gchar *)"/Options/_Wrap", (gchar *)"", menuSig, IDM_WRAP, (gchar *)"<CheckItem>"},
	            {(gchar *)"/Options/Wrap Out_put", (gchar *)"", menuSig, IDM_WRAPOUTPUT, (gchar *)"<CheckItem>"},
	            {(gchar *)"/Options/_Read-Only", (gchar *)"", menuSig, IDM_READONLY, (gchar *)"<CheckItem>"},
	            {(gchar *)"/Options/sep1", NULL, NULL, 0, (gchar *)"<Separator>"},
	            {(gchar *)"/Options/_Line End Characters", (gchar *)"", 0, 0, (gchar *)"<Branch>"},
	            {(gchar *)"/Options/Line End Characters/CR _+ LF", (gchar *)"", menuSig, IDM_EOL_CRLF, (gchar *)"<RadioItem>"},
	            {(gchar *)"/Options/Line End Characters/_CR", (gchar *)"", menuSig, IDM_EOL_CR, (gchar *)"/Options/Line End Characters/CR + LF"},
	            {(gchar *)"/Options/Line End Characters/_LF", (gchar *)"", menuSig, IDM_EOL_LF, (gchar *)"/Options/Line End Characters/CR + LF"},
	            {(gchar *)"/Options/_Convert Line End Characters", (gchar *)"", menuSig, IDM_EOL_CONVERT, 0},
	            {(gchar *)"/Options/sep2", NULL, NULL, 0, (gchar *)"<Separator>"},
	            {(gchar *)"/Options/Use _Monospaced Font", (gchar *)"<control>F11", menuSig, IDM_MONOFONT, (gchar *)"<CheckItem>"},
	            {(gchar *)"/Options/sep3", NULL, NULL, 0, (gchar *)"<Separator>"},
	            {(gchar *)"/Options/Open Local _Options File", (gchar *)"", menuSig, IDM_OPENLOCALPROPERTIES, 0},
	            {(gchar *)"/Options/Open _User Options File", (gchar *)"", menuSig, IDM_OPENUSERPROPERTIES, 0},
	            {(gchar *)"/Options/Open _Global Options File", (gchar *)"", menuSig, IDM_OPENGLOBALPROPERTIES, 0},
	            {(gchar *)"/Options/Open A_bbreviations File", (gchar *)"", menuSig, IDM_OPENABBREVPROPERTIES, 0},
	            {(gchar *)"/Options/sep4", NULL, NULL, 0, (gchar *)"<Separator>"},
	            {(gchar *)"/Options/Edit Properties", (gchar *)"", 0, 0, (gchar *)"<Branch>"},
	        };
	...
	SciTEItemFactoryEntry  menuItemsBuffer[] = {
	                                            {(gchar *)"/_Buffers", NULL, NULL, 0, (gchar *)"<Branch>"},
	                                            {(gchar *)"/Buffers/_Previous", (gchar *)"<shift>F6", menuSig, IDM_PREVFILE, 0},
	                                            {(gchar *)"/Buffers/_Next", (gchar *)"F6", menuSig, IDM_NEXTFILE, 0},
	                                            {(gchar *)"/Buffers/_Close All", (gchar *)"", menuSig, IDM_CLOSEALL, 0},
	                                            {(gchar *)"/Buffers/_Save All", (gchar *)"", menuSig, IDM_SAVEALL, 0},
	                                            {(gchar *)"/Buffers/sep2", NULL, NULL, 0, (gchar *)"<Separator>"},
	                                            {(gchar *)"/Buffers/Buffer0", (gchar *)"<alt>1", menuSig, bufferCmdID + 0, (gchar *)"<RadioItem>"},
	                                            {(gchar *)"/Buffers/Buffer1", (gchar *)"<alt>2", menuSig, bufferCmdID + 1, (gchar *)"/Buffers/Buffer0"},
	                                            {(gchar *)"/Buffers/Buffer2", (gchar *)"<alt>3", menuSig, bufferCmdID + 2, (gchar *)"/Buffers/Buffer0"},
	                                            {(gchar *)"/Buffers/Buffer3", (gchar *)"<alt>4", menuSig, bufferCmdID + 3, (gchar *)"/Buffers/Buffer0"},
	                                            {(gchar *)"/Buffers/Buffer4", (gchar *)"<alt>5", menuSig, bufferCmdID + 4, (gchar *)"/Buffers/Buffer0"},
	                                            {(gchar *)"/Buffers/Buffer5", (gchar *)"<alt>6", menuSig, bufferCmdID + 5, (gchar *)"/Buffers/Buffer0"},
	                                            {(gchar *)"/Buffers/Buffer6", (gchar *)"<alt>7", menuSig, bufferCmdID + 6, (gchar *)"/Buffers/Buffer0"},
	                                            {(gchar *)"/Buffers/Buffer7", (gchar *)"<alt>8", menuSig, bufferCmdID + 7, (gchar *)"/Buffers/Buffer0"},
	                                            {(gchar *)"/Buffers/Buffer8", (gchar *)"<alt>9", menuSig, bufferCmdID + 8, (gchar *)"/Buffers/Buffer0"},
	                                            {(gchar *)"/Buffers/Buffer9", (gchar *)"<alt>0", menuSig, bufferCmdID + 9, (gchar *)"/Buffers/Buffer0"},
	                                        };

	SciTEItemFactoryEntry  menuItemsHelp[] = {
	                                          {(gchar *)"/_Help", NULL, NULL, 0, (gchar *)"<Branch>"},
	                                          {(gchar *)"/Help/Quick _Help", (gchar *)"F1", menuSig, IDM_HELP, 0},
	                                          {(gchar *)"/Help/_sdlBasic Reference Guide", (gchar *)"", menuSig, IDM_HELP_SCITE, 0},
	                                          {(gchar *)"/Help/_About sdlBasic IDE", (gchar *)"", menuSig, IDM_ABOUT, 0},
	                                      };
...
void SciTEGTK::CreateUI() {
	...
	gtk_signal_connect(GTK_OBJECT(PWidget(wTabBar)),"button-release-event",GTK_SIGNAL_FUNC(GtkTabBarSwitch),NULL);
#endif
	tabVisible = false;
...
void SciTEGTK::CheckForRunningInstance(int argc, char *argv[]) {
	...
	const SString pipeFilename = props.Get("ipc.sdlBasic.name");

	if (pipeFilename.size() > 0)
		snprintf(pipeName, sizeof(pipeName), "%s", pipeFilename.c_str());
	else
		snprintf(pipeName, sizeof(pipeName), "%s/.sdlBasic.%s.ipc", g_get_tmp_dir (), g_get_user_name());
