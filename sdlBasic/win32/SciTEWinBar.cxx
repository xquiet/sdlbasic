...
// sdlbasic editor version
// this version of scite was create as editor for sdlbasic language
...
void SciTEWin::Notify(SCNotification *notification) {
	...
	case NM_RCLICK:
		...
		if (notification->nmhdr.idFrom == IDM_TABWIN) {
			...
			AddToPopUp("Find Next", IDM_FINDNEXT, true);
			AddToPopUp("Toggle Bookmark", IDM_BOOKMARK_TOGGLE, true);
			AddToPopUp("Previous Bookmark", IDM_BOOKMARK_PREV, true);
			AddToPopUp("Next Bookmark", IDM_BOOKMARK_NEXT, true);
			AddToPopUp("Go", IDM_GO, true);
			AddToPopUp("Stop", IDM_STOPEXECUTE, true);
	...
	case TTN_GETDISPINFO:
		// Ask for tooltip text
		{
			...
			case IDM_BOOKMARK_TOGGLE:
				ttext = "Toggle Bookmark";
				break;
			case IDM_BOOKMARK_PREV:
				ttext = "Previous Bookmark";
				break;
...
*/
	...
	::SendMessage(hwndToolBar, TB_LOADIMAGES, IDB_STD_LARGE_COLOR, reinterpret_cast<LPARAM>(HINST_COMMCTRL));

	TBADDBITMAP addbmp_filenew = { hInstance, IDR_FILENEW };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp_filenew);

	TBADDBITMAP addbmp_fileopen = { hInstance, IDR_FILEOPEN };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp_fileopen);

	TBADDBITMAP addbmp_filesave = { hInstance, IDR_FILESAVE };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp_filesave);

	TBADDBITMAP addbmp_closefile = { hInstance, IDR_CLOSEFILE };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp_closefile);

	TBADDBITMAP addbmp_undo = { hInstance, IDR_UNDO };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp_undo);

	TBADDBITMAP addbmp_redo = { hInstance, IDR_REDO };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp_redo);

	TBADDBITMAP addbmp_find = { hInstance, IDR_FIND };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp_find);

	TBADDBITMAP addbmp_findnext = { hInstance, IDR_FINDNEXT };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp_findnext);

	TBADDBITMAP addbmp_replace = { hInstance, IDR_REPLACE };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp_replace);

	TBADDBITMAP addbmp_togglebookmark = { hInstance, IDR_BOOKMARK_TOGGLE };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp_togglebookmark);

	TBADDBITMAP addbmp_previousbookmark = { hInstance, IDR_BOOKMARK_PREV };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp_previousbookmark);

	TBADDBITMAP addbmp_nextbookmark = { hInstance, IDR_BOOKMARK_NEXT };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp_nextbookmark);

	TBADDBITMAP addbmp_go = { hInstance, IDR_GO };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp_go);

	TBADDBITMAP addbmp_stop = { hInstance, IDR_STOPEXECUTE };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp_stop);
	...
	for (unsigned int i = 0;i < ELEMENTS(bbs);i++) {
		switch(bbs[i].cmd){
			...
			break;
		    case IDM_CLOSE:
			tbb[i].iBitmap = STD_PRINT + 4;
			break;
		    case IDM_UNDO:
			tbb[i].iBitmap = STD_PRINT + 5;
			break;
		    case IDM_REDO:
			tbb[i].iBitmap = STD_PRINT + 6;
			break;
		    case IDM_FIND:
			tbb[i].iBitmap = STD_PRINT + 7;
			break;
		    case IDM_FINDNEXT:
			tbb[i].iBitmap = STD_PRINT + 8;
			break;
		    case IDM_REPLACE:
			tbb[i].iBitmap = STD_PRINT + 9;
			break;
		    case IDM_BOOKMARK_TOGGLE:
			tbb[i].iBitmap = STD_PRINT + 10;
			break;
		    case IDM_BOOKMARK_PREV:
			tbb[i].iBitmap = STD_PRINT + 11;
			break;
		    case IDM_BOOKMARK_NEXT:
			tbb[i].iBitmap = STD_PRINT + 12;
			break;
		    case IDM_GO:
			tbb[i].iBitmap = STD_PRINT + 13;
			break;
		    case IDM_STOPEXECUTE:
			tbb[i].iBitmap = STD_PRINT + 14;
			break;
